...
export default function SampleList({ refreshKey, highlightId, onDeleted }) {
  ...
  async function onDelete(id) {
    if (!window.confirm('Hapus sampel ini?')) return
    try {
    ...
    <section className="bg-surface-container-lowest rounded-lg p-space-lg shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-space-sm mb-space-md">
        <div className="flex items-center gap-space-xs">
          ...
          {demo
            ? <span className="font-data-mono-sm px-2 py-0.5 rounded bg-[#fef3c7] text-[#b45309] font-bold">MODE DEMO LOKAL</span>
            : <span className="font-data-mono-sm px-2 py-0.5 rounded bg-secondary-container/40 text-on-secondary-container font-bold">SUPABASE LIVE</span>}
        </div>
        <div className="flex items-center gap-space-xs">
      ...
      {!isSupabaseConfigured && (
        <p className="font-body-sm text-[#b45309] bg-[#fef3c7] rounded px-3 py-2 mb-space-md">.env belum diisi — data disimpan di browser. Isi VITE_SUPABASE_URL + VITE_SUPABASE_ANON_KEY lalu restart dev server.</p>
      )}
              ...
              {filtered.map((r) => (
                <tr key={r.id} className={r.id === highlightId ? 'bg-secondary-container/30' : 'hover:bg-surface-container-low'}>
                  <td className="py-space-sm px-space-md whitespace-nowrap">
                    ...
                    <span className="font-data-mono-sm text-on-surface-variant">{r.matrix} • {r.priority}</span>
                  </td>
                  ...
                  <td className="py-space-sm px-space-md">
                    ...
                    <span className="font-data-mono-sm text-on-surface-variant">{r.storage_unit} • {r.rack_row}-{r.rack_col} • {r.status}</span>
                  </td>
