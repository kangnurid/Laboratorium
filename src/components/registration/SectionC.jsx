...
export default function SectionC() {
  ...
  const [note, setNote] = useState('Spesimen diantar kurir medik khusus RS Siloam dalam coolbox bersegel QC dengan 2 ice gel aktif.')
  const toggle = (i) => setChecks((p) => p.map((v, x) => (x === i ? !v : v)))
  ...
  return (
    <section className="bg-surface-container-lowest rounded-lg p-space-lg shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-space-md mb-space-md">
        <div className="p-space-sm rounded bg-surface-container-low flex flex-col gap-1">
          ...
          <div className="flex items-baseline gap-1">
            <input defaultValue="4.5" className="w-20 h-8 px-2 rounded bg-surface-container-lowest text-primary font-data-mono-lg font-bold" type="text" />
        ...
        <div className="p-space-sm rounded bg-surface-container-low flex flex-col gap-1">
          ...
          <div className="flex items-baseline gap-1">
            <input defaultValue="3.8" className="w-20 h-8 px-2 rounded bg-surface-container-lowest text-primary font-data-mono-lg font-bold" type="text" />
        ...
        <div className="p-space-sm rounded bg-surface-container-low flex flex-col gap-1">
          ...
          <select className="w-full h-8 px-2 rounded bg-surface-container-lowest font-body-sm font-semibold" defaultValue="Jernih (Non-Hemolisis)">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-space-sm mb-space-md">
        {labels.map((l, i) => (
          <label key={l} className="flex items-center gap-2 p-2.5 rounded bg-surface-container-low/60 cursor-pointer hover:bg-surface-container-low">
            <input checked={checks[i]} onChange={() => toggle(i)} className="w-4 h-4 accent-secondary" type="checkbox" />
      ...
      <div className="flex flex-col gap-space-xs">
        ...
        <input value={note} onChange={(e) => setNote(e.target.value)} className="w-full h-9 px-3 rounded bg-surface-container-low font-body-sm focus:outline-none focus:ring-1 focus:ring-primary" type="text" />
      </div>
    </section>
  )
}
